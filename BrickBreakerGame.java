...
public class BrickBreakerGame extends Applet implements Runnable, KeyListener{ 
  ...
  public void run (){  
    while (true){
      ...
      if(y_pos+radius >= yP && x_pos >= xP && x_pos <= xP+paddleWidth) {
      ...
      //hits bricks
      for(int i=0; i<6; i++){
        if ((y_pos == 150-radius || y_pos == radius+180) && (x_pos>i*100 && x_pos<i*100+100 && displayBricks[i] == 1)) {
          points += 1;
          brickCounter +=1;
          displayBricks[i] = 0;
          y_speed *= -1;
        ...
        else if ((x_pos==i*100+radius || x_pos==i*100+100+radius) && (y_pos>=150-radius && y_pos<=180+radius && displayBricks[i] == 1)) {
          points += 1;
          brickCounter +=1;
          displayBricks[i] = 0;
          x_speed *= -1;
